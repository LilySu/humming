...
template <
    ...
    uint32_t kNumBitsA,
    bool kPackedInput,
    bool kShouldPreprocessForINT2FP,
    bool kShouldPreprocessWithZP,
    bool kShouldTransposeMiniBlock,
    uint32_t kGroupSizeZP,
    bool kUsePackedKLayout = false,
    bool kUseNativeDequant = false,
    bool kUseSignedS4KMajorLayout = false>
...
__global__ void weight_repack_nk(
  ...
  if constexpr (kUseSignedS4KMajorLayout) {
    static_assert(kNumBitsB == 4, "signed-S4 K-major layout requires 4-bit weight");
    static_assert(kNumBitsA == 8, "signed-S4 K-major layout requires 8-bit activation");
    static_assert(!kShouldPreprocessForINT2FP, "signed-S4 K-major layout is int-only, not int->fp");
    static_assert(!kShouldPreprocessWithZP, "signed-S4 K-major layout requires a symmetric weight, no zero point");
